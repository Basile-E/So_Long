...
{
	if (x < 0 || y < 0 || x >= max_x || y >= max_y || map[y][x] == '1'
		|| visited[y][x])
		return (0);
	if (map[y][x] == 'E')
		return (1);
	visited[y][x] = 1;
	if (dfs(map, visited, x + 1, y, max_x, max_y) || dfs(map, visited, x - 1, y,
			max_x, max_y) || dfs(map, visited, x, y + 1, max_x, max_y)
...
int	is_valid_map(t_game *game)
{
	...
	if (!dfs(game->map, visited, game->player_x_pos, game->player_y_pos,
			game->cols, game->rows))
	{
